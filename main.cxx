...
int main() {
  ...
  std::function<void(void)> using_simd = []() {
	...
	for (size_t i = 0; i < ARRAY_SIZE; i += 4) {
	  packed = _mm_set_epi32(array[i], array[i + 1], array[i + 2], array[i + 3]);
	  indexes = _mm_set_epi32(i, i + 1, i + 2, i + 3);
	  result = _mm_add_epi32(packed, indexes);
  ...
  std::cout << "Average time Using SIMD:\t" << Avg(simd_times) << " microseconds\n";
